...
public class Camera2BasicFragment extends Fragment
    ...
    private static final String TAG = "Camera2BasicFragment";// 로그찍을때 태그
    ...
    private int facingId=0; // 전면이 보일때 0
  ...
  /*  private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
    ...
    public void setOnImageAvailableListener(ImageReader.OnImageAvailableListener mOnImageAvailableListener){ // 이미지가 리스너를 통해 셋팅이 됌
    ...
    private void setUpCameraOutputs(int width, int height) { // facingId를 통하여 카메라를 전면 or 후면으로 보일지를 정해준다.
        Activity activity = getActivity();
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case  R.id.picture_change:
                ...
                if(facingId==CameraCharacteristics.LENS_FACING_FRONT){    //  카메라가 전면을 비추고 있다면 후면으로 바뀜
                    facingId=CameraCharacteristics.LENS_FACING_BACK;

                }else{ // 카메라가 후면을 비추고 있다면 전면으로 바뀜
                    facingId=CameraCharacteristics.LENS_FACING_FRONT;
                }

                //카메라 전환을 위해
                closeCamera();//껏다가
                openCamera(mTextureView.getWidth(), mTextureView.getHeight());//다시 킴
                //카메라 전환을 위해
                /*Activity activity = getActivity();
                if (null != activity) {
     ...
     * Saves a JPEG {@link Image} into the specified {@link File}.
     */
/*    private static class ImageUpLoader implements Runnable { //카메라 사진을 찍고 이미지가 메모리 위에 있을때 바로 데이터 베이스로 보내어 프로필 사진 설정하기
        ...
        *//**
        ...
        ImageUpLoader(Image image) { //ImageUpLoader(Image image, File file)
            mImage = image;
            //mFile = file;
        }

        @Override
        public void run() { //촬영한 이미지를 Firebase Cloud Stoage로 보내줌
            ...
            /////////////////////////
            *//*uploadTask.addOnFailureListener(new OnFailureListener() { // 데이터 베이스로 사진 업로드
                @Override
                public void onFailure(@NonNull Exception exception) {
                    // Handle unsuccessful uploads
                    Log.v(TAG, "데이터 베이스로 사진 업로드 실패");
                }
